...
with CTE as( select H.Hacker_id as H_Hacker_id, 
H.name as name, s.score as scores,
...
select H_Hacker_id,name,sum(scores) 
from CTE 
where rn=1 
group by H_Hacker_id,name 
order by sum(scores) desc,H_Hacker_id asc;
